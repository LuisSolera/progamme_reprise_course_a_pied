...
const Phase = Object.freeze({
  PREP: 'prep', RUNNING: 'running', TRANSITION: 'transition', FINISHED: 'finished',
});
...
export const TrackerComponent = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const clockDisplay = computed(() => isRunning.value ? formatClock(remaining.value) : '');
    ...
    function beginTransition() {
      ...
      playTransitionCue(() => { if (phase.value === Phase.TRANSITION) startStep(); });
    }
    ...
    function skipStep() {
      ...
      if (isTransition.value) { cancelTransition(); startStep(); return; }
      clearInterval(timerId);
      endStep();
    }
    ...
    return {
      steps, totalSteps, phase, Phase, currentIndex, remaining, paused,
      currentStep, nextStep, stepLabel, clockDisplay, progressPct,
    ...
    <div class="tracker-overlay" @click.self="close" role="dialog" aria-modal="true" aria-labelledby="tracker-heading">
      <div class="tracker-card">
        ...
        <div>
          ...
          <span v-else-if="isTransition && nextStep" class="tracker-type-badge" :data-type="nextStep.type">
            {{ nextStep.type === 'jog' ? '🏃 Course' : '🚶 Marche' }}
          </span>
          <span v-else-if="isRunning" class="tracker-type-badge" :data-type="currentStep.type">
            {{ currentStep.type === 'jog' ? '🏃 Course' : '🚶 Marche' }}
          </span>
        </div>
        <div id="tracker-heading" class="tracker-clock" :class="{ 'is-jog': isRunning && currentStep.type === 'jog' }" aria-live="polite">{{ clockDisplay }}</div>
        <div class="tracker-step-info">
          <p class="tracker-step-label">
            <template v-if="isPrep">Préparez-vous…</template>
            <template v-else-if="isTransition && nextStep">{{ nextStep.label }}</template>
          ...
          <p class="tracker-step-detail">
            <template v-if="isPrep || isTransition || isFinished">&nbsp;</template>
            <template v-else>{{ currentStep.detail }}</template>
          </p>
        </div>
        <div class="tracker-progress-track" v-if="isRunning" role="progressbar" :aria-valuenow="Math.round(progressPct)" aria-valuemin="0" aria-valuemax="100">
          <div class="tracker-progress-fill" :style="{ width: progressPct + '%' }" :data-type="currentStep.type"></div>
        </div>
        <div v-else class="tracker-progress-track"><div class="tracker-progress-fill" style="width:0%"></div></div>
        <div class="tracker-dots" aria-hidden="true">
          <span v-for="(step, i) in steps" :key="i" class="tracker-dot"
        ...
        <div class="tracker-controls" v-if="!isFinished && !isPrep">
          <button class="btn-pause" @click="togglePause" :disabled="isTransition">{{ paused ? '▶ Reprendre' : '⏸ Pause' }}</button>
          <button class="btn-skip" @click="skipStep">Passer →</button>
        </div>
        <button v-if="isFinished" class="btn-save" @click="saveSession">✅ Terminer et sauvegarder</button>
      </div>
    </div>
  `,
});